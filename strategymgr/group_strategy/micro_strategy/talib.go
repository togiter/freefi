...
func Ma(data []common.KLine, inTimePeriod int, maType int, priceTy int) []float64 {
	return gotart.MaArr(gotart.MaType(maType), reData(data, PriceType(priceTy), SortPositive), int64(inTimePeriod))
	//return gotalib.Ma(realData(data, PriceType(priceTy)), inTimePeriod, gotalib.MaType(maType))
}
...
func BBands(data []common.KLine, inTimePeriod int, deviation float64, maType int, priceTy int) (up, middle, low []float64) {
	return gotart.BBandsArr(gotart.MaType(maType), reData(data, PriceType(priceTy), SortPositive), int64(inTimePeriod), deviation, deviation)
	//return gotalib.BBands(realData(data,PriceType(priceTy)), inTimePeriod, deviation, deviation, gotalib.MaType(maType))
}

func Rsi(data []common.KLine, inTimePeriod int, priceTy int) []float64 {
	return gotart.RsiArr(reData(data, PriceType(priceTy), SortPositive), int64(inTimePeriod))
	//return gotalib.Rsi(reData(data, PriceType(priceTy),SortPositive), inTimePeriod)
}

func realData(data []common.KLine, priceTy PriceType) []float64 {
	return reData(data, priceTy, SortReverse)
}

func reData(data []common.KLine, priceTy PriceType, sort DataSort) []float64 {
	var inReal []float64
	if sort == SortReverse {
		for i := len(data) - 1; i >= 0; i-- {
			k := data[i]
