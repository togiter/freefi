...
type GroupStrategyExt struct {
	PrePassRate      float64 `json:"prePassRate"` //  次级通过率
	FomoLevel        int     `json:"fomoLevel"`   //交易等级/none,pre,pro
	DelayMicroStrate string  `json:"delayMicroStrate"`
	IsDictatorship   bool    `json:"isDictatorship"` //是否独裁策略
	//延时确认时间倍数,KPeriod
	DelayX    float64 `json:"delayX"`
	IsRequire bool    `json:"isRequire"` //是否必选策略
}
